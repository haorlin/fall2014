...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        Location finalDestination = _map
                .getLabel(segment.get(segment.size() - 1));
        Road prevRoad = _map.getLabel(segment.get(0), segment.get(1));
        double totalLength = prevRoad.length();
        for (int i = 1; i < segment.size() - 1; i++) {
            Road thisRoad = _map.getLabel(segment.get(i), segment.get(i + 1));
            if (prevRoad.toString().equals(thisRoad.toString())
                    && prevRoad.direction() == thisRoad.direction()) {
                totalLength = totalLength + thisRoad.length();
            } else {
                System.out.println(seq + ". Take " + prevRoad + " "
                        + prevRoad.direction().fullName() + " for "
                        + (Math.round(totalLength * 10.0) / 10.0) + " miles.");
                seq++;
                prevRoad = thisRoad;
                totalLength = thisRoad.length();
            }
        }
        System.out.println(seq + ". Take " + prevRoad + " "
                + prevRoad.direction().fullName() + " for "
                + (Math.round(totalLength * 10.0) / 10.0) + " miles" + " to "
                + finalDestination + ".");
        seq++;
        return seq;
    }
