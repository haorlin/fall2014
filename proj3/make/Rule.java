...
class Rule {
    ...
    void addCommands(List<String> commands) {
        if (!commands.isEmpty() && !_commands.isEmpty()) {
            error("error: Illegal State Exception!");
        }
    ...
    private void checkFinishedDependencies() {
        for (int dependencies : _depends.successors(this._vertex)) {
            if (!_depends.contains(getVertex())
                    || _depends.getLabel(dependencies).outOfDate()) {
    ...
    private boolean outOfDate() {
        ...
        for (int dependencies : _depends.successors(this._vertex)) {
            if (getTime() < _depends.getLabel(dependencies).getTime()) {
                return true;
            }
        }
        return false;
