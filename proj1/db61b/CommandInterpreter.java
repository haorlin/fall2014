...
class CommandInterpreter {
    ...
    Table selectClause() {
        ...
        Table[] tables = tablelist.toArray(new Table[tablelist.size()]);
        if (_input.nextIf("where")) {
            conditions = conditionClause(tables);
        }
        Table tempTable;
        if (table2 == null) {
            tempTable = table.select(cols, conditions);
        } else {
            tempTable = table.select(table2, cols, conditions);
        }
        return tempTable;
    }
    ...
    Condition condition(Table... tables) {
        ...
        Column c1 = new Column(col1name, tables);
        String relation = _input.next(Tokenizer.RELATION);
        Condition cond;
        if (_input.nextIs(Tokenizer.LITERAL)) {
            String v2 = literal();
            cond = new Condition(c1, relation, v2);
        } else {
            String col2name = name();
            Column c2 = new Column(col2name, tables);
