...
abstract class Board extends Observable {
    ...
    boolean isLegal(Side player, int r, int c) {
        return player.playableSquare(get(r, c).getSide());
    }
    ...
    final Side getWinner() {
        if (numOfSide(whoseMove().opposite()) == size() * size()) {
    ...
    public String toString() {
        ...
        for (int i = 0; i < size(); i++) {
            ...
            for (int j = 0; j < size(); j++) {
                boardString += get(i + 1, j + 1).getSpots();
                String side = "";
                if (get(i + 1, j + 1).getSide() == Side.WHITE) {
                    side = "-";
                } else if (get(i + 1, j + 1).getSide() == Side.BLUE) {
                    side = "b";
                } else {
                    side = "r";
                }
